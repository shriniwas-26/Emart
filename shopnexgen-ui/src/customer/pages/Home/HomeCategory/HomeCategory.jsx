...
const HomeCategory = () => {
  ...
  // Use local data if API data is not available
  const categoriesToShow = homePage.homePageData?.shopByCategories || homeCategory;
  
  return (
    <div className='flex justify-center gap-3 flex-wrap '>
        {categoriesToShow
          .slice(0, isSmallScreen ? 6 : homeCategory.length)
          .map((item, index) => (
            <HomeCategoryCard key={`${item.categoryId}_${index}`} item={item}/>
          ))}
